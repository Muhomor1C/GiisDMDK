...
Файлы с данными о товаре были переведены в CSV для быстроты и удобства работы. В итоге получил файл END.CSV,
в котором имеются все необходимые данные, сопоставленные между собой. Эта работа не отражена в репозитории.
Желающим могу вылсать утилиту, которая была написана для этого.

  В конечном итоге, работник сканирует заводской ярлык изделия и принтер автоматически печатает этикетку с УИН кодом
в виде DanaMatrix. Кроме этого, на этикетке печатается наименование изделия, вес общий и отдельно вес драг. металлов, оригинальный
...
товара(5000-7000 позиций).

  Что не сделано(TODO):
  Утилита должна работать на всех принтерах TSC, но проверить нет возможности. Как и нет возможности поэксперементировать с другими
  производителями.
